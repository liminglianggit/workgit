...
function  auto_quote(req)
	...
	if json.Get(req.para,'/insurance/chengke') ~= nil then
		...
		if temp > 200000 then
			...
			req.para = json.Set(req.para,"/msg",'乘客险保额超出限额，最高保额为20万！');
	...
	if json.Get(req.para,'/insurance/siji') ~= nil then
		...
		if temp > 200000 then
			...
			req.para = json.Set(req.para,"/error",-3);
			req.para = json.Set(req.para,"/msg",'司机险保额超出限额，最高保额为20万！');
			req:complete(req.para);
			return false;
		end
	end
	...
	if  flag == nil then
		...
		req:complete("{\"error\":-4,\"msg\":\"进入投保报价页面失败！\"}");
		return false;
	end
	...
	repeat
	until(web.ExecuteScript(page, "document.getElementById('customerName').value;")); 
