...
function  auto_quote(req)
	...
	repeat
		...
		web.ExecuteScript(page,"document.getElementsByClassName('btn btn-operation btn-display ng-binding')[0].click();\r\n\
			external.put_result('flag','1');\r\n\
			")
		sys.DBGPrint("点击进入首页中......\r\n");
		flag = web.GetResult("flag");
		if flag == '1' then
			sys.DBGPrint("点击进入首页成功！"..((web.GetTickCount()-startTime)/1000).."\r\n");
			break;
		end
	until(web.IsTimeout(tm1))

	if  flag == nil then
		sys.DBGPrint("点击进入首页失败！\r\n");
		web.ClosePage(page);
		req.para = json.Set(req.para,"/error",-5);
		req.para = json.Set(req.para,"/msg",'点击进入首页失败！!');
		req:complete(req.para);
		return false;
	...
	repeat
		web.Wait(500);
		web.ExecuteScript(page,"if (document.getElementsByClassName('ng-binding ng-scope')[0]){\r\n\
	...
	repeat
		web.ExecuteScript(page,"document.getElementsByClassName('ng-binding ng-scope')[0].click();\r\n\
								external.put_result('flag','1');\r\n\
			");
		sys.DBGPrint("点击签单管理页面中......\r\n");
		flag = web.GetResult("flag");
		if flag == '1' then
			sys.DBGPrint("点击签单管理页面成功！"..((web.GetTickCount()-startTime)/1000).."\r\n");
			break;
		end
		web.Wait(100);
	until(web.IsTimeout(tm1))
	...
	repeat
		web.ExecuteScript(page,"document.getElementById('vinbutton').disabled=false;\r\n\
			...
			");
		web.Wait(100);
		sys.DBGPrint("点击车架查询中......\r\n");
		flag = web.GetResult("flag");
		if flag == '1' then
			sys.DBGPrint("点击车架查询成功"..((web.GetTickCount()-startTime)/1000).."\r\n");
			break;
		end
	...
	if  flag == nil then
		web.ClosePage(page);
		sys.DBGPrint("点击车架查询失败\r\n");
		req:complete("{\"error\":-4,\"msg\":\"点击车架查询失败\"}");
		return false;
	end
